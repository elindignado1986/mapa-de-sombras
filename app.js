...
const state={mode:'sun',minutes:960,date:'2026-06-21',width:36,depth:30,rotation:0,yaw:-.35,pitch:.85,zoom:2.8,flat:false,frontage:35,base:"streets",panX:0,panY:0};
...
function setView(flat){state.flat=flat;state.yaw=flat?0:-.35;state.pitch=.85;state.panX=0;state.panY=0;state.zoom=flat?1:2.8;for(const [id,active]of [['view2d',flat],['view3d',!flat]]){$(id).classList.toggle('active',active);$(id).setAttribute('aria-pressed',String(active));}document.querySelector('.maphint').textContent=flat?'Arrastrá para desplazar · dos dedos para zoom':'Arrastrá para girar · dos dedos para zoom y desplazar';canvas.style.cursor=flat?'grab':'move';draw();}
$('view2d').onclick=()=>setView(true);$('view3d').onclick=()=>setView(false);$('zoomIn').onclick=()=>{state.zoom=Math.min(5,state.zoom*1.3);draw()};$('zoomOut').onclick=()=>{state.zoom=Math.max(.5,state.zoom/1.3);draw()};$('reset').onclick=()=>setView(state.flat);
// Track each contact independently so a second finger starts a pinch, not a rotation.
const activePointers=new Map();
const clampZoom=value=>Math.max(.5,Math.min(5,value));
