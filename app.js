...
function zoomAt(next,x=W*.5,y=H*.5){const old=state.zoom;next=Math.max(window.Urban?.active ? .006 : .5,Math.min(window.Urban?.active?30:5,next));if(window.Urban?.active&&state.flat){const ratio=next/old;state.panX=x-W*.5-(x-W*.5-state.panX)*ratio;state.panY=y-H*.54-(y-H*.54-state.panY)*ratio;}state.zoom=next;}
$('view2d').onclick=()=>setView(true);$('view3d').onclick=()=>setView(false);$('zoomIn').onclick=()=>{zoomAt(state.zoom*1.3);draw()};$('zoomOut').onclick=()=>{zoomAt(state.zoom/1.3);draw()};$('reset').onclick=()=>setView(state.flat);
...
canvas.onwheel=e=>{e.preventDefault();const rect=canvas.getBoundingClientRect();zoomAt(state.zoom*Math.exp(-e.deltaY*.001),e.clientX-rect.left,e.clientY-rect.top);draw()};canvas.onkeydown=e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','-'].includes(e.key)){e.preventDefault();if(state.flat){if(e.key==='ArrowLeft')state.panX+=20;if(e.key==='ArrowRight')state.panX-=20;if(e.key==='ArrowUp')state.panY+=20;if(e.key==='ArrowDown')state.panY-=20}else{if(e.key==='ArrowLeft')state.yaw-=.12;if(e.key==='ArrowRight')state.yaw+=.12;if(e.key==='ArrowUp')state.pitch=Math.max(.25,state.pitch-.08);if(e.key==='ArrowDown')state.pitch=Math.min(1.2,state.pitch+.08)}if(e.key==='+')zoomAt(state.zoom*1.1);if(e.key==='-')zoomAt(state.zoom/1.1);draw()}};
