...
$('view2d').onclick=()=>setView(true);$('view3d').onclick=()=>setView(false);$('zoomIn').onclick=()=>{state.zoom=Math.min(window.Urban?.active?30:5,state.zoom*1.3);draw()};$('zoomOut').onclick=()=>{state.zoom=Math.max(.5,state.zoom/1.3);draw()};$('reset').onclick=()=>setView(state.flat);
// Track each contact independently so a second finger starts a pinch, not a rotation.
const activePointers=new Map();
const clampZoom=value=>Math.max(.5,Math.min(window.Urban?.active?30:5,value));
...
canvas.onwheel=e=>{e.preventDefault();state.zoom=Math.max(.5,Math.min(window.Urban?.active?30:5,state.zoom*Math.exp(-e.deltaY*.001)));draw()};canvas.onkeydown=e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','-'].includes(e.key)){e.preventDefault();if(state.flat){if(e.key==='ArrowLeft')state.panX+=20;if(e.key==='ArrowRight')state.panX-=20;if(e.key==='ArrowUp')state.panY+=20;if(e.key==='ArrowDown')state.panY-=20}else{if(e.key==='ArrowLeft')state.yaw-=.12;if(e.key==='ArrowRight')state.yaw+=.12;if(e.key==='ArrowUp')state.pitch=Math.max(.25,state.pitch-.08);if(e.key==='ArrowDown')state.pitch=Math.min(1.2,state.pitch+.08)}if(e.key==='+')state.zoom=Math.min(window.Urban?.active?30:5,state.zoom*1.1);if(e.key==='-')state.zoom=Math.max(.5,state.zoom/1.1);draw()}};
